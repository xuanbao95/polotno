...
export default function DemoSticky({ store }) {
                ...
                if (data.items) {
                    return data.items.map((item) => {
                        ...
                        <div onClick={async () => {
                                ...
                                const req = await fetch(`/partner/${item.json}`);
    
                                const json = await req.json();
                                // just inject it into store
                                store.loadJSON(json);
                                console.log(json);
                            }} style={{backgroundImage:`url('/partner/${item.preview}')`,backgroundPosition:"center",backgroundSize:"contain",backgroundRepeat:"no-repeat",height:"100px",width:"100%"}}></div>
                    </div>)
                    })
                }
            })
    ...
    return (
        <div className="slickAll">
            ...
            getPreview={(item) => `/partner/${item.preview}`}
            onSelect={async (item) => {
              // download selected json
              const req = await fetch(`/partner/${item.json}`);
              const json = await req.json();
